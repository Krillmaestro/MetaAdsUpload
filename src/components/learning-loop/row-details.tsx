...
import { ExternalLink, FlaskConical, Link2, Unlink, Loader2 } from "lucide-react";
import { toast } from "sonner";
import { cn } from "@/lib/utils";
import type { LearningLoopRow } from "@/lib/learning-loop/rows";
import { CLASSIFICATION_CONFIG } from "@/lib/evolve/classifier";
import { fmtMoney, fmtNum, fmtPct, fmtX, fmtDate, AWARENESS_LEVELS, STATUS_LABEL } from "./format";
    ...
    <div className="grid gap-4 border-t border-white/5 bg-[#0b0f1c] px-4 py-4 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,1.4fr)_minmax(0,1.2fr)]" onClick={(e) => e.stopPropagation()}>
      ...
      <div className="space-y-2">
        ...
        <div className={cn("rounded-lg border px-3 py-2 text-xs", cls.bg, cls.border)}>
          ...
          {row.isTopSpender && <span className="ml-2 text-slate-500">· top spender i kampanjen ({fmtPct(row.spendShare * 100, 0)})</span>}
        </div>
        {row.ads.length > 0 && (
