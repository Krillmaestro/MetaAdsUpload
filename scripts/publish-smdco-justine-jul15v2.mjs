...
// pages (PP + ITCH2) = 6 ads. Same pipeline as publish-smdco-justine-jul15v2.mjs
...
const PAGE = "164161996779020";             // Dr. Ben Stone
...
const SRC_DIR = "/Users/kristoffermacbook/Downloads/BATCH 15.V2";
// Filnamnens "TBD"-slot ersätts med LP-nyckeln per annons (PP/ITCH2), som Fervin Jul13.
const ADSET_NAME = "SE Justine Jul15.V2 - #1 - VSL - PP + ITCH2 - Evergreen - Scratching - Oskar";
const AD_NAME = (hook, lp) => `${hook} SE Justine Jul15.V2 - #1 - VSL - ${lp} - Evergreen - Scratching - Oskar`;
const EDITOR_NAME = "Justine";
const BATCH_NUMBER = "Jul15.V2";

const LINKS = [
  { key: "PP", url: "https://thesmalldogco.com/" },
  { key: "ITCH2", url: "https://thesmalldogco.com/pages/itch2" },
];
...
const STATE_FILE = `${STATE_DIR}/smdco-justine-jul15v2-state.json`;
  ...
  try {
    await sql`INSERT INTO upload_jobs (filename, media_type, status, total_steps, current_step, step_label,
              ...
              VALUES (${v.file}, 'video', 'completed', 4, 4, 'Klar!', ${vid.r2Key}, ${vid.r2Url}, ${CAMP},
                      ...
                      ${JSON.stringify({ adName: name, adCopy: { headlines: HEADLINES, primaryTexts: TEXTS, linkUrl: link.url, ctaType: CTA }, pageId: PAGE, pixelId: PIXEL, adAccountId: ACT, source: "publish-smdco-justine-jul15v2" })}::jsonb,
                      now())`;
  } catch (e) { console.warn(`    (upload_jobs-rad misslyckades, icke-kritiskt: ${e.message})`); }
}

// ── run ──────────────────────────────────────────────────────────────────────
console.log(`\n=== SmallDogCO Justine Jul15.V2 ${DRY ? "(DRY RUN — inga skrivningar)" : "LIVE LAUNCH"} ===`);
console.log(`Kampanj: SmallDogCO Itch Relief // ABO (${CAMP})  |  konto ${ACT}`);
console.log(`Template ${TEMPLATE_ID}: ${tpl.name}  |  sida ${PAGE} (Dr. Ben Stone)  |  pixel ${PIXEL}`);
