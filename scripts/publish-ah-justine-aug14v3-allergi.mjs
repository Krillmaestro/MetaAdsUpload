...
// ── ApotekHunden Allergi & Klåda — Justine Aug14.V3 (SWEDISH) ──────────────
// 1 ad set into "Allergi & Klåda // CBO // 2026-04" (CBO → ingen ad set-budget).
// 3 hooks × 2 landing pages (LP1 + LP1A från template 8 "Allergi & Klåda #1 + #1A")
// = 6 ads. SE, 18-65, advantage audience, alla placeringar, 7d-klick-attribution,
// destination WEBSITE, default-exkludering av audience 120250065842190350.
// Sida: Apotek Hunden (explicit begäran 2026-08-16).
const DRY = process.env.DRY_RUN === "1";

const ACT = "act_261297039993717";          // Glimmora (ApotekHunden)
const CAMP = "120247704475550350";           // Allergi & Klåda // CBO // 2026-04
const PIXEL = "1485774658810931";            // ApotekHunden-pixeln
const PAGE = "265790413295490";             // Apotek Hunden
const TEMPLATE_ID = 8;                       // "Allergi & Klåda #1 + #1A"
const CTA = "SHOP_NOW";
...
const SRC_DIR = "/Users/kristoffermacbook/Downloads/BATCH #14.V3 15.12.39";
// Filnamnens LP-slot är tom → fylls med LP1/LP1A (LP1 = templatens #1-sida,
// verifierad mot "SE Krille Jun23 ... LP1"-adsen i samma kampanj).
const ADSET_NAME = "SE Justine Aug14.V3 - #0 - VSL - LP1 + LP1A - Evergreen - MIX - Krille";
const AD_NAME = (hook, lp) => `${hook} SE Justine Aug14.V3 - #0 - VSL - ${lp} - Evergreen - MIX - Krille`;
const EDITOR_NAME = "Justine";
const BATCH_NUMBER = "Aug14.V3";

const LINKS = [
  { key: "LP1", url: "https://www.apotekhunden.se/pages/allergi-och-klada1?_ab=0&key=1776797690955" },
  { key: "LP1A", url: "https://www.apotekhunden.se/pages/allergi-and-klada-1a?_ab=0&key=1776797605403" },
];
...
const STATE_FILE = `${STATE_DIR}/ah-justine-aug14v3-allergi-state.json`;
  ...
  try {
    await sql`INSERT INTO upload_jobs (filename, media_type, status, total_steps, current_step, step_label,
              ...
              VALUES (${v.file}, 'video', 'completed', 4, 4, 'Klar!', ${vid.r2Key}, ${vid.r2Url}, ${CAMP},
                      ...
                      ${JSON.stringify({ adName: name, adCopy: { headlines: HEADLINES, primaryTexts: TEXTS, linkUrl: link.url, ctaType: CTA }, pageId: PAGE, pixelId: PIXEL, adAccountId: ACT, source: "publish-ah-justine-aug14v3-allergi" })}::jsonb,
                      now())`;
  } catch (e) { console.warn(`    (upload_jobs-rad misslyckades, icke-kritiskt: ${e.message})`); }
}

// ── run ──────────────────────────────────────────────────────────────────────
console.log(`\n=== ApotekHunden Justine Aug14.V3 SWEDISH ${DRY ? "(DRY RUN — inga skrivningar)" : "LIVE LAUNCH"} ===`);
console.log(`Kampanj: Allergi & Klåda // CBO // 2026-04 (${CAMP})  |  konto ${ACT} (Glimmora)`);
console.log(`Template ${TEMPLATE_ID}: ${tpl.name}  |  sida ${PAGE} (Apotek Hunden)  |  pixel ${PIXEL}`);
console.log(`Ad set: ${ADSET_NAME}`);
console.log(`CBO — ingen ad set-budget  |  SE, 18-65, advantage audience, alla placeringar  |  attribution 7d-klick  |  status ${STATUS}`);
