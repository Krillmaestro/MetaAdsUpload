...
const ROOT = "/Users/kristoffermacbook/Downloads";
const BATCHES = [
  { dir: "BATCH 16.V1 SWEDISH", slot: "LP11 + PP", editor: "Florencio", batch: "Aug16.V1" },
];
const LINKS = [
...
const STATE_FILE = `${STATE_DIR}/ah-aug16v1-launch-state.json`;
...
for (const b of BATCHES) {
  ...
  const adsetName = withSlot(baseName(h1).replace(/^H\d+\s+/i, ""), b.slot, "LP11 + LP12");
  plan.push({ ...b, dir, videos, adsetName });
}

console.log(`\n=== BATCH 16.V1 → ProBiotics // ABO ${DRY ? "(DRY RUN)" : "LIVE LAUNCH"} ===`);
console.log(`Template ${TEMPLATE_ID}: ${tpl.name} | sida ${PAGE} | pixel ${PIXEL} | ${(DAILY_BUDGET/100)} kr/dag per ad set | status ${STATUS}`);
console.log(`Landers: LP11 hundhalsa-emma (SHOP_NOW) + LP12 (SHOP_NOW)\n`);
for (const p of plan) {
    ...
    try {
      await sql`INSERT INTO upload_jobs (filename, media_type, status, total_steps, current_step, step_label, r2_key, r2_url, campaign_id, adset_id, ad_id, video_id, config, completed_at)
                VALUES (${v.file}, 'video', 'completed', 4, 4, 'Klar!', ${vid.r2Key}, ${vid.r2Url}, ${CAMPAIGN_ID}, ${bs.adsetId}, ${r.id}, ${vid.videoId},
                        ${JSON.stringify({ adName: name, adCopy: { headlines: HEADLINES, primaryTexts: TEXTS, linkUrl: link.url, ctaType: link.cta }, pageId: PAGE, pixelId: PIXEL, adAccountId: ACT, source: "publish-ah-florencio-aug16v1-probiotika" })}::jsonb, now())`;
