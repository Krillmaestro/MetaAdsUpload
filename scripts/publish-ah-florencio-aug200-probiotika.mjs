...
// ── ApotekHunden Probiotika — Florencio Aug200 (BATCH 198.V2) ─────────────────────
...
const SRC_DIR = "/Users/kristoffermacbook/Downloads/Batch 200";
// Filnamnens LP-slot är tom → fylls med PP/LP12 per annons.
const ADSET_NAME = "SE Florencio Aug200 - #1 - VSL - PP + LP12 - Evergreen - MIX - Oskar";
const AD_NAME = (hook, lp) => `${hook} SE Florencio Aug200 - #1 - VSL - ${lp} - Evergreen - MIX - Oskar`;
const EDITOR_NAME = "Florencio";
const BATCH_NUMBER = "Aug200";
...
const STATE_FILE = `${STATE_DIR}/ah-florencio-aug200-probiotika-state.json`;
  ...
  try {
    await sql`INSERT INTO upload_jobs (filename, media_type, status, total_steps, current_step, step_label,
              ...
              VALUES (${v.file}, 'video', 'completed', 4, 4, 'Klar!', ${vid.r2Key}, ${vid.r2Url}, ${state.campaignId},
                      ...
                      ${JSON.stringify({ adName: name, adCopy: { headlines: HEADLINES, primaryTexts: TEXTS, linkUrl: link.url, ctaType: CTA }, pageId: PAGE, pixelId: PIXEL, adAccountId: ACT, source: "publish-ah-florencio-aug200-probiotika" })}::jsonb,
                      now())`;
  } catch (e) { console.warn(`    (upload_jobs-rad misslyckades, icke-kritiskt: ${e.message})`); }
}

// ── run ──────────────────────────────────────────────────────────────────────
console.log(`\n=== ApotekHunden Probiotika Florencio Aug200 ${DRY ? "(DRY RUN — inga skrivningar)" : "LIVE LAUNCH"} ===`);
