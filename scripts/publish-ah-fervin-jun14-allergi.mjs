...
// ── ApotekHunden Allergi & Klåda — Fervin Jun14 (SWEDISH) ──────────────
...
const SRC_DIR = "/Users/kristoffermacbook/Downloads/ALLERGI 14";
...
const ADSET_NAME = "SE Fervin Jun14 - #0 - UGC - LP1 + LP1A - Flash - Scratching - Krille";
const AD_NAME = (hook, lp) => `${hook} SE Fervin Jun14 - #0 - UGC - ${lp} - Flash - Scratching - Krille`;
const EDITOR_NAME = "Fervin";
const BATCH_NUMBER = "Jun14";
...
const STATE_FILE = `${STATE_DIR}/ah-fervin-jun14-allergi-state.json`;
  ...
  try {
    await sql`INSERT INTO upload_jobs (filename, media_type, status, total_steps, current_step, step_label,
              ...
              VALUES (${v.file}, 'video', 'completed', 4, 4, 'Klar!', ${vid.r2Key}, ${vid.r2Url}, ${CAMP},
                      ...
                      ${JSON.stringify({ adName: name, adCopy: { headlines: HEADLINES, primaryTexts: TEXTS, linkUrl: link.url, ctaType: CTA }, pageId: PAGE, pixelId: PIXEL, adAccountId: ACT, source: "publish-ah-fervin-jun14-allergi" })}::jsonb,
                      now())`;
  } catch (e) { console.warn(`    (upload_jobs-rad misslyckades, icke-kritiskt: ${e.message})`); }
}

// ── run ──────────────────────────────────────────────────────────────────────
console.log(`\n=== ApotekHunden Fervin Jun14 SWEDISH ${DRY ? "(DRY RUN — inga skrivningar)" : "LIVE LAUNCH"} ===`);
