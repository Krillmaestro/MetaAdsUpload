...
const TODAY = JSON.stringify({ since: "2026-09-06", until: "2026-09-06" });
const ALLERGI_GY = "120252287364240350", PRO_GY = "120254770556580350";

const data = JSON.parse(readFileSync("/private/tmp/claude-501/-Users-kristoffermacbook/149ad702-f80b-43ed-9cd9-742554db01ef/scratchpad/abo-data-sep6.json", "utf8"));
const all = data.flatMap((c) => c.adsets.map((s) => ({ ...s })));
const want = [
  { key: "Statics #10 - DARFOR", gy: ALLERGI_GY },
  { key: "Statics #7 - RONTGEN", gy: PRO_GY },
];
...
for (const w of want) {
  ...
  if (roasToday > 2.0) { console.log("  🛡️ ÖVER 2 I DAG — RÖRS INTE (spärren)"); continue; }
