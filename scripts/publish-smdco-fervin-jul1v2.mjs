...
// pages (PP + ITCH2) = 6 ads. Same pipeline as publish-smdco-fervin-jul1v2.mjs
...
const SRC_DIR = "/Users/kristoffermacbook/Downloads/SMALL DOG 1.V2";
// Filnamnens "TBD"-slot ersätts med LP-nyckeln per annons (PP/ITCH2), som Fervin Jul13.
const ADSET_NAME = "USA Fervin Jul1.V2 - #1 - VSL - PP + ITCH2 - Evergreen - Scratching - Oskar";
const AD_NAME = (hook, lp) => `${hook} USA Fervin Jul1.V2 - #1 - VSL - ${lp} - Evergreen - Scratching - Oskar`;
const EDITOR_NAME = "Fervin";
const BATCH_NUMBER = "Jul1.V2";
...
const STATE_FILE = `${STATE_DIR}/smdco-fervin-jul1v2-state.json`;
  ...
  try {
    await sql`INSERT INTO upload_jobs (filename, media_type, status, total_steps, current_step, step_label,
              ...
              VALUES (${v.file}, 'video', 'completed', 4, 4, 'Klar!', ${vid.r2Key}, ${vid.r2Url}, ${CAMP},
                      ...
                      ${JSON.stringify({ adName: name, adCopy: { headlines: HEADLINES, primaryTexts: TEXTS, linkUrl: link.url, ctaType: CTA }, pageId: PAGE, pixelId: PIXEL, adAccountId: ACT, source: "publish-smdco-fervin-jul1v2" })}::jsonb,
                      now())`;
  } catch (e) { console.warn(`    (upload_jobs-rad misslyckades, icke-kritiskt: ${e.message})`); }
}

// ── run ──────────────────────────────────────────────────────────────────────
console.log(`\n=== SmallDogCO Fervin Jul1.V2 ${DRY ? "(DRY RUN — inga skrivningar)" : "LIVE LAUNCH"} ===`);
