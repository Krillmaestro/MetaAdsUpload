...
// ── ApotekHunden Munhälsa — Fervin Aug19 (MUNHALSA 22) ───────────────────────
...
const SRC_DIR = "/Users/kristoffermacbook/Downloads/Munhälsa launch/MUNHALSA 19";
const ADSET_NAME = "SE Fervin Aug19 - #0 - UGC - PP + 5R - Evergreen - OralHealth - Oskar";
const AD_NAME = (hook, lp) => `${hook} SE Fervin Aug19 - #0 - UGC - ${lp} - Evergreen - OralHealth - Oskar`;
const EDITOR_NAME = "Fervin";
const BATCH_NUMBER = "Aug19";
...
const STATE_FILE = `${STATE_DIR}/ah-fervin19-aug19-munhalsa-state.json`;
  ...
  try {
    await sql`INSERT INTO upload_jobs (filename, media_type, status, total_steps, current_step, step_label,
              ...
              VALUES (${v.file}, 'video', 'completed', 4, 4, 'Klar!', ${vid.r2Key}, ${vid.r2Url}, ${CAMP},
                      ...
                      ${JSON.stringify({ adName: name, adCopy: { headlines: HEADLINES, primaryTexts: TEXTS, linkUrl: link.url, ctaType: CTA }, pageId: PAGE, pixelId: PIXEL, adAccountId: ACT, source: "publish-ah-fervin19-aug19-munhalsa" })}::jsonb,
                      now())`;
  } catch (e) { console.warn(`    (upload_jobs-rad misslyckades, icke-kritiskt: ${e.message})`); }
}

// ── run ──────────────────────────────────────────────────────────────────────
console.log(`\n=== ApotekHunden Munhälsa Fervin Aug19 ${DRY ? "(DRY RUN — inga skrivningar)" : "LIVE LAUNCH"} ===`);
