...
// ── ApotekHunden Allergi & Klåda — Fervin Aug14 (SWEDISH) ──────────────
// 1 ad set into "Allergi & Klåda // CBO // 2026-04" (CBO → ingen ad set-budget).
// 3 hooks × 2 landing pages (LP1 + PP från template 8 "Allergi & Klåda #1 + #1A")
...
const TEMPLATE_ID = 30;                       // "Allergi & Klåda #1 + #1A"
const CTA = "SHOP_NOW";
...
const SRC_DIR = "/Users/kristoffermacbook/Downloads/BATCH #14 Allergi";
...
const ADSET_NAME = "SE Fervin Aug14 - #0 - VSL - LP1 + PP - Evergreen - MIX - Krille";
const AD_NAME = (hook, lp) => `${hook} SE Fervin Aug14 - #0 - VSL - ${lp} - Evergreen - MIX - Krille`;
const EDITOR_NAME = "Fervin";
const BATCH_NUMBER = "Aug14";

const LINKS = [
  { key: "LP1", url: "https://www.apotekhunden.se/pages/allergi-och-klada1?_ab=0&key=1776797690955" },
  { key: "PP", url: "https://www.apotekhunden.se/products/klada-allergi" },
];
...
const STATE_FILE = `${STATE_DIR}/ah-fervin-aug14-review-allergi-state.json`;
  ...
  try {
    await sql`INSERT INTO upload_jobs (filename, media_type, status, total_steps, current_step, step_label,
              ...
              VALUES (${v.file}, 'video', 'completed', 4, 4, 'Klar!', ${vid.r2Key}, ${vid.r2Url}, ${CAMP},
                      ...
                      ${JSON.stringify({ adName: name, adCopy: { headlines: HEADLINES, primaryTexts: TEXTS, linkUrl: link.url, ctaType: CTA }, pageId: PAGE, pixelId: PIXEL, adAccountId: ACT, source: "publish-ah-fervin-aug14-review-allergi" })}::jsonb,
                      now())`;
  } catch (e) { console.warn(`    (upload_jobs-rad misslyckades, icke-kritiskt: ${e.message})`); }
}

// ── run ──────────────────────────────────────────────────────────────────────
console.log(`\n=== ApotekHunden Fervin Aug14 SWEDISH ${DRY ? "(DRY RUN — inga skrivningar)" : "LIVE LAUNCH"} ===`);
