...
// ── ApotekHunden Probiotika — Fervin Jun194 (BATCH 29.V6) ─────────────────────
...
const SRC_DIR = "/Users/kristoffermacbook/Downloads/PRB launch/BATCH 194";
// Filnamnens LP-slot är tom → fylls med PP/LP12 per annons.
const ADSET_NAME = "SE Fervin Jun194 - #1 - VSL - LP12 + PP - Evergreen - MIX - Oskar";
const AD_NAME = (hook, lp) => `${hook} SE Fervin Jun194 - #1 - VSL - ${lp} - Evergreen - MIX - Oskar`;
const EDITOR_NAME = "Fervin";
const BATCH_NUMBER = "Jun194";
...
const STATE_FILE = `${STATE_DIR}/ah-fervin-jun194-probiotika-state.json`;
  ...
  try {
    await sql`INSERT INTO upload_jobs (filename, media_type, status, total_steps, current_step, step_label,
              ...
              VALUES (${v.file}, 'video', 'completed', 4, 4, 'Klar!', ${vid.r2Key}, ${vid.r2Url}, ${state.campaignId},
                      ...
                      ${JSON.stringify({ adName: name, adCopy: { headlines: HEADLINES, primaryTexts: TEXTS, linkUrl: link.url, ctaType: CTA }, pageId: PAGE, pixelId: PIXEL, adAccountId: ACT, source: "publish-ah-fervin-jun194-probiotika" })}::jsonb,
                      now())`;
  } catch (e) { console.warn(`    (upload_jobs-rad misslyckades, icke-kritiskt: ${e.message})`); }
}

// ── run ──────────────────────────────────────────────────────────────────────
console.log(`\n=== ApotekHunden Probiotika Fervin Jun194 ${DRY ? "(DRY RUN — inga skrivningar)" : "LIVE LAUNCH"} ===`);
