...
// ── ApotekHunden Probiotika — Fervin Jun197 (BATCH 199 (CARTOON)) ─────────────────────
...
const SRC_DIR = "/Users/kristoffermacbook/Downloads/Probiotika launch/BATCH 197";
// Filnamnens LP-slot är tom → fylls med PP/LP12 per annons.
const ADSET_NAME = "SE Fervin Jun197 - #1 - UGC - PP + LP12 - Evergreen - MIX - Oskar";
const AD_NAME = (hook, lp) => `${hook} SE Fervin Jun197 - #1 - UGC - ${lp} - Evergreen - MIX - Oskar`;
const EDITOR_NAME = "Fervin";
const BATCH_NUMBER = "Jun197";
...
const STATE_FILE = `${STATE_DIR}/ah-fervin-jun197-probiotika-state.json`;
  ...
  try {
    await sql`INSERT INTO upload_jobs (filename, media_type, status, total_steps, current_step, step_label,
              ...
              VALUES (${v.file}, 'video', 'completed', 4, 4, 'Klar!', ${vid.r2Key}, ${vid.r2Url}, ${state.campaignId},
                      ...
                      ${JSON.stringify({ adName: name, adCopy: { headlines: HEADLINES, primaryTexts: TEXTS, linkUrl: link.url, ctaType: CTA }, pageId: PAGE, pixelId: PIXEL, adAccountId: ACT, source: "publish-ah-fervin-jun197-probiotika" })}::jsonb,
                      now())`;
  } catch (e) { console.warn(`    (upload_jobs-rad misslyckades, icke-kritiskt: ${e.message})`); }
}

// ── run ──────────────────────────────────────────────────────────────────────
console.log(`\n=== ApotekHunden Probiotika Fervin Jun197 ${DRY ? "(DRY RUN — inga skrivningar)" : "LIVE LAUNCH"} ===`);
