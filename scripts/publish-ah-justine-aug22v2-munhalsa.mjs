...
// ── ApotekHunden Munhälsa — Justine Aug22 (MUNHALSA 22) ───────────────────────
...
// default-exkludering av audience 120250065842190350. Sida: Apotek Hunden.
...
const SRC_DIR = "/Users/kristoffermacbook/Downloads/BATCH 22.V2";
const ADSET_NAME = "Justine Aug22 - #0 - VSL - PP + 5R - Evergreen - OralHealth - Oskar";
const AD_NAME = (hook, lp) => `${hook} Justine Aug22 - #0 - VSL - ${lp} - Evergreen - OralHealth - Oskar`;
const EDITOR_NAME = "Justine";
const BATCH_NUMBER = "Aug22";
...
const STATE_FILE = `${STATE_DIR}/ah-justine-aug22v2-munhalsa-state.json`;
  ...
  try {
    await sql`INSERT INTO upload_jobs (filename, media_type, status, total_steps, current_step, step_label,
              ...
              VALUES (${v.file}, 'video', 'completed', 4, 4, 'Klar!', ${vid.r2Key}, ${vid.r2Url}, ${CAMP},
                      ...
                      ${JSON.stringify({ adName: name, adCopy: { headlines: HEADLINES, primaryTexts: TEXTS, linkUrl: link.url, ctaType: CTA }, pageId: PAGE, pixelId: PIXEL, adAccountId: ACT, source: "publish-ah-justine-aug22-munhalsa" })}::jsonb,
                      now())`;
  } catch (e) { console.warn(`    (upload_jobs-rad misslyckades, icke-kritiskt: ${e.message})`); }
}

// ── run ──────────────────────────────────────────────────────────────────────
console.log(`\n=== ApotekHunden Munhälsa Justine Aug22 ${DRY ? "(DRY RUN — inga skrivningar)" : "LIVE LAUNCH"} ===`);
