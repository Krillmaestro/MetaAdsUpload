...
// ── batch config ──
const DIR = "C:/Users/krill/Downloads/VIDEO-20260710T203933Z-2-001/VIDEO";
const ADSET_NAME = "USA Justine Jul11 - #1 - UGC - PP+LP - Evergreen - Scratching - Krille";
const COMBINED = "PP+LP"; // substring in the ad-set name replaced per LP for ad names
const LPS = [
  { token: "PP", url: "https://www.dogdivaco.com/products/diva-digest" },
  { token: "LP", url: "https://www.dogdivaco.com/" },
];
const TAG = "justine";

// ── shared (same as DivaDigest launches) ──
const ACT = "act_2277004866371824";
const CAMP = "120251818258290782";        // DivaDigest // CBO
const PAGE = "1216938401498787";
const PIXEL = "3401593933335351";
...
const STATE_FILE = `C:/Users/krill/AppData/Local/Temp/claude/C--Users-krill/3790fc75-c98f-4e85-866c-d93d63404fd2/scratchpad/dd-${TAG}-state.json`;

const files = readdirSync(DIR).filter((f) => /\.mp4$/i.test(f));
const VIDEOS = files.map((f) => { const m = f.match(/^(H\d+)/i); return { hook: (m ? m[1] : f).toUpperCase(), file: f }; })
  .sort((a, b) => (parseInt(a.hook.slice(1)) || 0) - (parseInt(b.hook.slice(1)) || 0));
const adName = (hook, token) => `${hook} ${ADSET_NAME.replace(COMBINED, token)}`;
console.log(`Justine: ${VIDEOS.length} videos × ${LPS.length} LP = ${VIDEOS.length * LPS.length} ads`);
...
async function uploadVideo(v) {
  ...
  const up = await gPostJson(`/${ACT}/advideos`, { file_url: publicUrl, title: `${v.hook} ${ADSET_NAME}` });
...
async function createAdSet() {
  ...
  const r = await gPostJson(`/${ACT}/adsets`, {
    campaign_id: CAMP, name: ADSET_NAME, optimization_goal: "OFFSITE_CONVERSIONS", billing_event: "IMPRESSIONS",
    destination_type: "WEBSITE", promoted_object: { pixel_id: PIXEL, custom_event_type: "PURCHASE" },
    attribution_spec: [{ event_type: "CLICK_THROUGH", window_days: 7 }, { event_type: "VIEW_THROUGH", window_days: 1 }, { event_type: "ENGAGED_VIDEO_VIEW", window_days: 1 }],
...
async function createAd(v, lp) {
  const keyId = `${v.hook}-${lp.token}`;
  if (state.ads[keyId]?.adId) { console.log(`  ${keyId}: exists`); return; }
  const vid = state.videos[v.hook];
  const name = adName(v.hook, lp.token);
  const texts = [...TEXTS.map((text) => ({ text, text_type: "primary_text" })), ...HEADLINES.map((text) => ({ text, text_type: "headline" }))];
  const group = { texts, call_to_action: { type: CTA, value: { link: lp.url } }, videos: [{ video_id: vid.videoId, ...(vid.thumbnailUrl ? { thumbnail_url: vid.thumbnailUrl } : {}) }] };
  const storySpec = { page_id: PAGE, video_data: { video_id: vid.videoId, message: TEXTS[0] || "", title: HEADLINES[0] || "", link_description: HEADLINES[0] || "", ...(vid.thumbnailUrl ? { image_url: vid.thumbnailUrl } : {}), call_to_action: { type: CTA, value: { link: lp.url } } } };
  const form = new FormData();
  form.append("adset_id", state.adsetId); form.append("name", name); form.append("status", STATUS);
  ...
  state.ads[keyId] = { adId: r.id, name, url: lp.url }; save();
  console.log(`  ✓ ${keyId}: ${r.id}  "${name}"`);
}
...
console.log("\n── Phase 2: ad set ──");
await createAdSet();
console.log("\n── Phase 3: ads (video × LP) ──");
for (const v of VIDEOS) for (const lp of LPS) { try { await createAd(v, lp); } catch (e) { console.error(`  ✗ ${v.hook}-${lp.token}: ${e.message}`); } }

console.log("\n── SUMMARY ──");
console.log("Ad set:", state.adsetId, "—", ADSET_NAME);
for (const v of VIDEOS) for (const lp of LPS) { const a = state.ads[`${v.hook}-${lp.token}`]; console.log(`  ${a?.adId || "—"}  ${adName(v.hook, lp.token)}  → ${lp.url}`); }
