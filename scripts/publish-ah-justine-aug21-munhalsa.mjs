...
// ── ApotekHunden Munhälsa — Justine Aug21 (MUNHALSA 22) ───────────────────────
...
const SRC_DIR = "/Users/kristoffermacbook/Downloads/Batch 21";
const ADSET_NAME = "SE Justine Aug21 - #0 - VSL - PP + 5R - Evergreen - MIX - Oskar";
const AD_NAME = (hook, lp) => `${hook} SE Justine Aug21 - #0 - VSL - ${lp} - Evergreen - MIX - Oskar`;
const EDITOR_NAME = "Justine";
const BATCH_NUMBER = "Aug21";
...
const STATE_FILE = `${STATE_DIR}/ah-justine-aug21-munhalsa-state.json`;
  ...
  try {
    await sql`INSERT INTO upload_jobs (filename, media_type, status, total_steps, current_step, step_label,
              ...
              VALUES (${v.file}, 'video', 'completed', 4, 4, 'Klar!', ${vid.r2Key}, ${vid.r2Url}, ${CAMP},
                      ...
                      ${JSON.stringify({ adName: name, adCopy: { headlines: HEADLINES, primaryTexts: TEXTS, linkUrl: link.url, ctaType: CTA }, pageId: PAGE, pixelId: PIXEL, adAccountId: ACT, source: "publish-ah-justine-aug21-munhalsa" })}::jsonb,
                      now())`;
  } catch (e) { console.warn(`    (upload_jobs-rad misslyckades, icke-kritiskt: ${e.message})`); }
}

// ── run ──────────────────────────────────────────────────────────────────────
console.log(`\n=== ApotekHunden Munhälsa Justine Aug21 ${DRY ? "(DRY RUN — inga skrivningar)" : "LIVE LAUNCH"} ===`);
