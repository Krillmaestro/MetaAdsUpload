...
// ── ApotekHunden Munhälsa — Fervin Aug23 (MUNHALSA 23) ───────────────────────
...
const SRC_DIR = "/Users/kristoffermacbook/Downloads/MUNHALSA 23";
const ADSET_NAME = "SE Fervin Aug23 - #0 - ANIME - PP + 5R - Evergreen - OralHealth - Oskar";
const AD_NAME = (hook, lp) => `${hook} SE Fervin Aug23 - #0 - ANIME - ${lp} - Evergreen - OralHealth - Oskar`;
const EDITOR_NAME = "Fervin";
const BATCH_NUMBER = "Aug23";
...
const STATE_FILE = `${STATE_DIR}/ah-fervin-aug23-munhalsa-state.json`;
  ...
  try {
    await sql`INSERT INTO upload_jobs (filename, media_type, status, total_steps, current_step, step_label,
              ...
              VALUES (${v.file}, 'video', 'completed', 4, 4, 'Klar!', ${vid.r2Key}, ${vid.r2Url}, ${CAMP},
                      ...
                      ${JSON.stringify({ adName: name, adCopy: { headlines: HEADLINES, primaryTexts: TEXTS, linkUrl: link.url, ctaType: CTA }, pageId: PAGE, pixelId: PIXEL, adAccountId: ACT, source: "publish-ah-fervin-aug23-munhalsa" })}::jsonb,
                      now())`;
  } catch (e) { console.warn(`    (upload_jobs-rad misslyckades, icke-kritiskt: ${e.message})`); }
}

// ── run ──────────────────────────────────────────────────────────────────────
console.log(`\n=== ApotekHunden Munhälsa Fervin Aug23 ${DRY ? "(DRY RUN — inga skrivningar)" : "LIVE LAUNCH"} ===`);
