...
// ── ApotekHunden Munhälsa — Florencio Aug20 (MUNHALSA 22) ───────────────────────
...
const SRC_DIR = "/Users/kristoffermacbook/Downloads/Munhälsa launch/Batch 20 SWEDISH";
const ADSET_NAME = "SE Florencio Aug20 - #0 - VSL - PP + 5R - Evergreen - OralHealth - Oskar";
const AD_NAME = (hook, lp) => `${hook} SE Florencio Aug20 - #0 - VSL - ${lp} - Evergreen - OralHealth - Oskar`;
const EDITOR_NAME = "Florencio";
const BATCH_NUMBER = "Aug20";
...
const STATE_FILE = `${STATE_DIR}/ah-flo20-aug20-munhalsa-state.json`;
  ...
  try {
    await sql`INSERT INTO upload_jobs (filename, media_type, status, total_steps, current_step, step_label,
              ...
              VALUES (${v.file}, 'video', 'completed', 4, 4, 'Klar!', ${vid.r2Key}, ${vid.r2Url}, ${CAMP},
                      ...
                      ${JSON.stringify({ adName: name, adCopy: { headlines: HEADLINES, primaryTexts: TEXTS, linkUrl: link.url, ctaType: CTA }, pageId: PAGE, pixelId: PIXEL, adAccountId: ACT, source: "publish-ah-flo20-aug20-munhalsa" })}::jsonb,
                      now())`;
  } catch (e) { console.warn(`    (upload_jobs-rad misslyckades, icke-kritiskt: ${e.message})`); }
}

// ── run ──────────────────────────────────────────────────────────────────────
console.log(`\n=== ApotekHunden Munhälsa Florencio Aug20 ${DRY ? "(DRY RUN — inga skrivningar)" : "LIVE LAUNCH"} ===`);
