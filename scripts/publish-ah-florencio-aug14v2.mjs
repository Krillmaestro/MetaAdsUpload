...
// ── ApotekHunden Allergi & Klåda — Florencio Aug14.V2 (SWEDISH) ──────────────
...
// Sida: Josephine Hart (explicit begäran 2026-08-16).
...
const SRC_DIR = "/Users/kristoffermacbook/Downloads/BATCH #14.V2 SWEDISH";
...
const ADSET_NAME = "SE Florencio Aug14.V2 - #0 - VSL - LP1 + LP1A - Evergreen -";
const AD_NAME = (hook, lp) => `${hook} SE Florencio Aug14.V2 - #0 - VSL - ${lp} - Evergreen -`;
const EDITOR_NAME = "Florencio";
const BATCH_NUMBER = "Aug14.V2";
...
const STATE_FILE = `${STATE_DIR}/ah-florencio-aug14v2-state.json`;
  ...
  try {
    await sql`INSERT INTO upload_jobs (filename, media_type, status, total_steps, current_step, step_label,
              ...
              VALUES (${v.file}, 'video', 'completed', 4, 4, 'Klar!', ${vid.r2Key}, ${vid.r2Url}, ${CAMP},
                      ...
                      ${JSON.stringify({ adName: name, adCopy: { headlines: HEADLINES, primaryTexts: TEXTS, linkUrl: link.url, ctaType: CTA }, pageId: PAGE, pixelId: PIXEL, adAccountId: ACT, source: "publish-ah-florencio-aug14v2" })}::jsonb,
                      now())`;
  } catch (e) { console.warn(`    (upload_jobs-rad misslyckades, icke-kritiskt: ${e.message})`); }
}

// ── run ──────────────────────────────────────────────────────────────────────
console.log(`\n=== ApotekHunden Florencio Aug14.V2 SWEDISH ${DRY ? "(DRY RUN — inga skrivningar)" : "LIVE LAUNCH"} ===`);
