...
// ── ApotekHunden Probiotika — Florencio Aug202.V1 (BATCH 198.V2) ─────────────────────
...
// 4 hooks × 2 LPs (PP + LP12 från template 28 "PP + LP12 nya Primärtexterna")
...
const CAMPAIGN_NAME = "ProBiotics // ABO"; // befintlig kampanj 120229285210840350
...
const DAILY_BUDGET = 30000;                  // 300 kr/dag i öre (ABO — budget på ad set)
...
const SRC_DIR = "/Users/kristoffermacbook/Downloads/Probiotika launch/BATCH 202.V1 SWEDISH";
// Filnamnens LP-slot är tom → fylls med PP/LP12 per annons.
const ADSET_NAME = "SE Florencio Aug202.V1 - #1 - Non narrated - PP + LP12 - Evergreen - MIX - Oskar";
const AD_NAME = (hook, lp) => `${hook} SE Florencio Aug202.V1 - #1 - Non narrated - ${lp} - Evergreen - MIX - Oskar`;
const EDITOR_NAME = "Florencio";
const BATCH_NUMBER = "Aug202.V1";

const LINKS = [
  { key: "PP", url: "https://www.apotekhunden.se/products/apotekhunden-probiotika" },
  { key: "LP12", url: "https://www.apotekhunden.se/pages/ahprobiotika?_ab=0&key=1776716014500" },
];
...
const STATE_FILE = `${STATE_DIR}/ah-florencio-aug202v1-probiotika-state.json`;
  ...
  try {
    await sql`INSERT INTO upload_jobs (filename, media_type, status, total_steps, current_step, step_label,
              ...
              VALUES (${v.file}, 'video', 'completed', 4, 4, 'Klar!', ${vid.r2Key}, ${vid.r2Url}, ${state.campaignId},
                      ...
                      ${JSON.stringify({ adName: name, adCopy: { headlines: HEADLINES, primaryTexts: TEXTS, linkUrl: link.url, ctaType: CTA }, pageId: PAGE, pixelId: PIXEL, adAccountId: ACT, source: "publish-ah-florencio-aug202v1-probiotika" })}::jsonb,
                      now())`;
  } catch (e) { console.warn(`    (upload_jobs-rad misslyckades, icke-kritiskt: ${e.message})`); }
}

// ── run ──────────────────────────────────────────────────────────────────────
console.log(`\n=== ApotekHunden Probiotika Florencio Aug202.V1 ${DRY ? "(DRY RUN — inga skrivningar)" : "LIVE LAUNCH"} ===`);
