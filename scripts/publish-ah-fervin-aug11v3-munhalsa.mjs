...
// ── ApotekHunden Munhälsa — Fervin Aug11.V3 (MUNHALSA 22) ───────────────────────
...
const SRC_DIR = "/Users/kristoffermacbook/Downloads/Munhälsa launch/Munhalsa BATCH 11.V3";
const ADSET_NAME = "SE Fervin Aug11.V3 - #0 - UGC - PP + 5R - Evergreen - OralHealth - Oskar";
const AD_NAME = (hook, lp) => `${hook} SE Fervin Aug11.V3 - #0 - UGC - ${lp} - Evergreen - OralHealth - Oskar`;
const EDITOR_NAME = "Fervin";
const BATCH_NUMBER = "Aug11.V3";
...
const STATE_FILE = `${STATE_DIR}/ah-fervin11v3-aug11v3-munhalsa-state.json`;
  ...
  try {
    await sql`INSERT INTO upload_jobs (filename, media_type, status, total_steps, current_step, step_label,
              ...
              VALUES (${v.file}, 'video', 'completed', 4, 4, 'Klar!', ${vid.r2Key}, ${vid.r2Url}, ${CAMP},
                      ...
                      ${JSON.stringify({ adName: name, adCopy: { headlines: HEADLINES, primaryTexts: TEXTS, linkUrl: link.url, ctaType: CTA }, pageId: PAGE, pixelId: PIXEL, adAccountId: ACT, source: "publish-ah-fervin11v3-aug11v3-munhalsa" })}::jsonb,
                      now())`;
  } catch (e) { console.warn(`    (upload_jobs-rad misslyckades, icke-kritiskt: ${e.message})`); }
}

// ── run ──────────────────────────────────────────────────────────────────────
console.log(`\n=== ApotekHunden Munhälsa Fervin Aug11.V3 ${DRY ? "(DRY RUN — inga skrivningar)" : "LIVE LAUNCH"} ===`);
