...
// ── SmallDogCO DirtyEars — Batch 9 (Justine Jul6) ────────────────────────────
// 1 ad set into "SmallDogCO Itch Relief // ABO" at $35/day. 3 hooks × 1 landing
// page (PP) = 3 ads. Same pipeline and settings as publish-smdco-batch10.mjs
// (US/CA/AU, 18-65, advantage audience, mobile FB+IG, 7d-click/1d-view/1d-EV),
// but on the Josephine Hart page instead of Dr. Kathrina Lindley.
...
const SRC_DIR = "/Users/kristoffermacbook/Downloads/BATCH 9";
// Exact names per Kristoffer (note the double space after "PP "), matching the filenames.
const ADSET_NAME = "USA Justine Jul6 - #1 - VSL - PP  - Evergreen - DirtyEars - Oskar";
const AD_NAME = (hook) => `${hook} USA Justine Jul6 - #1 - VSL - PP  - Evergreen - DirtyEars - Oskar`;
const EDITOR_NAME = "Justine";
const BATCH_NUMBER = "Jul6";
...
const STATE_FILE = `${STATE_DIR}/smdco-batch9-justine-state.json`;
  ...
  try {
    await sql`INSERT INTO upload_jobs (filename, media_type, status, total_steps, current_step, step_label,
              ...
              VALUES (${v.file}, 'video', 'completed', 4, 4, 'Klar!', ${vid.r2Key}, ${vid.r2Url}, ${CAMP},
                      ...
                      ${JSON.stringify({ adName: name, adCopy: { headlines: HEADLINES, primaryTexts: TEXTS, linkUrl: link.url, ctaType: CTA }, pageId: PAGE, pixelId: PIXEL, adAccountId: ACT, source: "publish-smdco-batch9-justine" })}::jsonb,
                      now())`;
  } catch (e) { console.warn(`    (upload_jobs-rad misslyckades, icke-kritiskt: ${e.message})`); }
}

// ── run ──────────────────────────────────────────────────────────────────────
console.log(`\n=== SmallDogCO Batch 9 Justine ${DRY ? "(DRY RUN — inga skrivningar)" : "LIVE LAUNCH"} ===`);
...
console.log(`\n${videos.length} videor × ${LINKS.length} landningssida = ${videos.length * LINKS.length} annonser:`);
for (const v of videos) {
  const mb = (statSync(`${SRC_DIR}/${v.file}`).size / 1024 / 1024).toFixed(0);
  console.log(`  ${v.hook} (${mb}MB) ${v.file}`);
  for (const l of LINKS) console.log(`     → ${AD_NAME(v.hook)}  →  ${l.url}`);
}
