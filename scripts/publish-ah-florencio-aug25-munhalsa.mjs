...
// ── ApotekHunden Munhälsa — Florencio Aug25 (MUNHALSA 22) ───────────────────────
...
const SRC_DIR = "/Users/kristoffermacbook/Downloads/BATCH 25 SWEDISH";
const ADSET_NAME = "SE Florencio Aug25 - #0 - UGC - PP + 5R - Evergreen - OralHealth - Oskar";
const AD_NAME = (hook, lp) => `${hook} SE Florencio Aug25 - #0 - UGC - ${lp} - Evergreen - OralHealth - Oskar`;
const EDITOR_NAME = "Florencio";
const BATCH_NUMBER = "Aug25";
...
const STATE_FILE = `${STATE_DIR}/ah-florencio-aug25-aug25-munhalsa-state.json`;
  ...
  try {
    await sql`INSERT INTO upload_jobs (filename, media_type, status, total_steps, current_step, step_label,
              ...
              VALUES (${v.file}, 'video', 'completed', 4, 4, 'Klar!', ${vid.r2Key}, ${vid.r2Url}, ${CAMP},
                      ...
                      ${JSON.stringify({ adName: name, adCopy: { headlines: HEADLINES, primaryTexts: TEXTS, linkUrl: link.url, ctaType: CTA }, pageId: PAGE, pixelId: PIXEL, adAccountId: ACT, source: "publish-ah-florencio-aug25-aug25-munhalsa" })}::jsonb,
                      now())`;
  } catch (e) { console.warn(`    (upload_jobs-rad misslyckades, icke-kritiskt: ${e.message})`); }
}

// ── run ──────────────────────────────────────────────────────────────────────
console.log(`\n=== ApotekHunden Munhälsa Florencio Aug25 ${DRY ? "(DRY RUN — inga skrivningar)" : "LIVE LAUNCH"} ===`);
