...
// ── ApotekHunden Munhälsa — Justine Aug18 (MUNHALSA 22) ───────────────────────
...
// default-exkludering av audience 120250065842190350. Sida: Hund Hälsan.
...
const PAGE = "483622214837771";              // Hund Hälsan
...
const SRC_DIR = "/Users/kristoffermacbook/Downloads/Munhälsa launch/BATCH 18";
const ADSET_NAME = "SE Justine Aug18 - #0 - VSL - PP + 5R - Evergreen - OralHealth - Oskar";
const AD_NAME = (hook, lp) => `${hook} SE Justine Aug18 - #0 - VSL - ${lp} - Evergreen - OralHealth - Oskar`;
const EDITOR_NAME = "Justine";
const BATCH_NUMBER = "Aug18";
...
const STATE_FILE = `${STATE_DIR}/ah-justine18-aug18-munhalsa-state.json`;
  ...
  try {
    await sql`INSERT INTO upload_jobs (filename, media_type, status, total_steps, current_step, step_label,
              ...
              VALUES (${v.file}, 'video', 'completed', 4, 4, 'Klar!', ${vid.r2Key}, ${vid.r2Url}, ${CAMP},
                      ...
                      ${JSON.stringify({ adName: name, adCopy: { headlines: HEADLINES, primaryTexts: TEXTS, linkUrl: link.url, ctaType: CTA }, pageId: PAGE, pixelId: PIXEL, adAccountId: ACT, source: "publish-ah-justine18-aug18-munhalsa" })}::jsonb,
                      now())`;
  } catch (e) { console.warn(`    (upload_jobs-rad misslyckades, icke-kritiskt: ${e.message})`); }
}

// ── run ──────────────────────────────────────────────────────────────────────
console.log(`\n=== ApotekHunden Munhälsa Justine Aug18 ${DRY ? "(DRY RUN — inga skrivningar)" : "LIVE LAUNCH"} ===`);
console.log(`Kampanj: Munhälsa // CBO (${CAMP})  |  konto ${ACT} (Glimmora)`);
console.log(`Template ${TEMPLATE_ID}: ${tpl.name}  |  sida ${PAGE} (Hund Hälsan)  |  pixel ${PIXEL}`);
