...
// ── SmallDogCO Itch Relief — Batch 10 (Florencio Jul10) ──────────────────────
// 1 ad set into "SmallDogCO Itch Relief // ABO" at $35/day. 3 hooks × 2 landing
// pages (PP + EAR1) = 6 ads. Settings mirror the live ad sets created Jul 29
// (US/CA/AU, 18-65, advantage audience, mobile FB+IG, 7d-click/1d-view/1d-EV).
//
// The web uploader cannot express US+CA+AU — its country dropdown only offers
// single countries or BIG5, so the template's "CORE3" is passed through raw and
// Meta rejects it (that is what killed the Jul 27 attempt, error #100 "Invalid
// country code"). Same pipeline as the app otherwise: R2 → /advideos by URL →
// creative_asset_groups_spec (Flexible Ads) with the template's copy.
...
const TEMPLATE_ID = 22;                     // SmallDogCO Allergy & Itch EARISSUES
const CTA = "SHOP_NOW";
const STATUS = "ACTIVE";                    // launch live, per Kristoffer
const DAILY_BUDGET = 3500;                  // $35.00/day (cents)
// The live SmallDogCO ads on this page use utm_source=fb (the app default is
// "facebook"); matching the neighbours keeps this brand's reporting uniform.
const URL_TAGS = "utm_source=fb&utm_medium=paid&utm_campaign={{campaign.id}}&utm_term={{adset.id}}&utm_content={{ad.id}}";

const SRC_DIR = "/Users/kristoffermacbook/Downloads/Batch 10 SmallDogco";
const ADSET_NAME = "USA Florencio Jul10 - #1 - VSL - PP + EAR1 - Evergreen - DirtyEars - Oskar";
const AD_NAME = (hook, lp) => `${hook} USA Florencio Jul10 - #1 - VSL - ${lp} - Evergreen - DirtyEars - Oskar`;
const EDITOR_NAME = "Florencio";
const BATCH_NUMBER = "Jul10";

const LINKS = [
  { key: "PP", url: "https://thesmalldogco.com/" },
  { key: "EAR1", url: "https://thesmalldogco.com/pages/ears1" },
];

const STATE_DIR = "/private/tmp/claude-501/-Users-kristoffermacbook/9634d719-6f57-482e-9aca-f8c19ca992a8/scratchpad";
if (!existsSync(STATE_DIR)) mkdirSync(STATE_DIR, { recursive: true });
const STATE_FILE = `${STATE_DIR}/smdco-batch10-state.json`;
...
async function uploadVideo(v) {
  ...
  // Same key convention as the app's presign route (purpose=library)
  ...
  if (!state.videos[v.hook]?.r2Key) {
    ...
    // Mirror the app: register the file in the Creative Library
    try {
...
async function createAd(v, link) {
  ...
  // Mirror the app: log the upload so it shows in /upload-log
  try {
    await sql`INSERT INTO upload_jobs (filename, media_type, status, total_steps, current_step, step_label,
              ...
              VALUES (${v.file}, 'video', 'completed', 4, 4, 'Klar!', ${vid.r2Key}, ${vid.r2Url}, ${CAMP},
                      ...
                      ${JSON.stringify({ adName: name, adCopy: { headlines: HEADLINES, primaryTexts: TEXTS, linkUrl: link.url, ctaType: CTA }, pageId: PAGE, pixelId: PIXEL, adAccountId: ACT, source: "publish-smdco-batch10" })}::jsonb,
                      now())`;
  } catch (e) { console.warn(`    (upload_jobs-rad misslyckades, icke-kritiskt: ${e.message})`); }
}

// ── run ──────────────────────────────────────────────────────────────────────
console.log(`\n=== SmallDogCO Batch 10 ${DRY ? "(DRY RUN — inga skrivningar)" : "LIVE LAUNCH"} ===`);
